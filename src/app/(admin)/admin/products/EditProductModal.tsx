...
export default function EditProductModal({ product, onClose }: EditProductModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (priceInCents + negativeSum <= 0) {
      return toast.error(
        `Price would go negative with current adjustments. Increase base price or reduce discounts.`
      );
    }
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              ...
              className="px-6 py-2 bg-slate-900 hover:bg-black text-white font-medium rounded-md text-sm transition duration-150 disabled:bg-gray-400"
            >
